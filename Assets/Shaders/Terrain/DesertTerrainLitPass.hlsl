#ifndef DESERT_TERRAIN_LIT_PASS_INCLUDED
#define DESERT_TERRAIN_LIT_PASS_INCLUDED
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, float3 screenUVZ, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    half4 diffAlbedo[4];
    ...
    // CUSTOM: ----------------------------------------------------------------------------
    // Flow sands
    float biasA = frac(_Time.y * _SandFlowSpeed);
    float biasB = biasA + 0.5;
    float facB = abs(biasA - 0.5) * 2.0;
    half3 sandFlowA = SampleSandFlowTexture(uvSplat01.xy + float2(biasA, biasA));
    half3 sandFlowB = SampleSandFlowTexture(uvSplat01.xy + float2(biasB, biasB));
    half3 sandMaskA = SampleSandMaskTexture(uvSplat01.xy + sandFlowA * _SandFlowSpeed);
    half3 sandMaskB = SampleSandMaskTexture(uvSplat01.xy + sandFlowB * _SandFlowSpeed);
    half3 sandMask = lerp(sandMaskA, sandMaskB, facB);
    half3 flowSandColor = _SandMaskColor * sandMask;
    
    // Flash sands
    half3 flashNoiseSurface = SampleFlashNoiseTexture(uvSplat01.xy);
    half3 flashNoiseScreen = SampleFlashNoiseTexture(screenUVZ.xy);
    half3 flash = step(0.01, flashNoiseSurface * flashNoiseScreen);
    flash = step(_FlashThreshold, (flashNoiseSurface * flashNoiseScreen + length(flowSandColor) * flashNoiseSurface)) *
        (1.0 - screenUVZ.z) * flash * 3.0;
    // ------------------------------------------------------------------------------------

    mixedDiffuse = 0.0h;
    mixedDiffuse += (diffAlbedo[0] + half4(flash + flowSandColor, 0.0)) *
        half4(_DiffuseRemapScale0.rgb * splatControl.rrr, 1.0h);
